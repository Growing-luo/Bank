...
<script type="text/javascript">
	...
	$(function(){
		$("#account").datagrid({
			...
			url:"AccountServlet?caozuo=initdatagrid"
		});
		...
		$("#add").click(function(){
			actionflag="add";
			$("#dlgaccount").dialog("open").dialog("center").dialog("setTitle","开户");
		...
		$("#save").click(function(){
			...
			if($("#regplace").textbox("getValue")==""){
				$.messager.alert("提示","未填写开户银行！","info");
				return;
			}
			if(actionflag=="add"){//add
				$.post("AccountServlet?caozuo=add",
		...
		$("#frozen").click(function(){
			...
			if(row=="" || row==null){
				$.messager.alert("提示","请选择要冻结的账户！","info");
				return;
			}
			$.post("AccountServlet?caozuo=frozen",
		...
		$("#unfrozen").click(function(){
			...
			$.post("AccountServlet?caozuo=unfrozen",
			{"account_id":row.account_id},
			function(data){
				var value=eval("("+data+")");
				if(value.status=="success"){
		...
		$("#cancel").click(function(){
			var row=$("#account").datagrid("getSelected");  //获得datagrid选中的行
			if(row=="" || row==null){
				$.messager.alert("提示","请选择要注销的客户信息！","info");
				return;
			}
			$.post("AccountServlet?caozuo=cancel",
			{"account_id":row.account_id},
			function(data){
				var value=eval("("+data+")");
				if(value.status=="success"){
 ...
 <body style="margin-top:10px; margin-left: 4px; margin-right:4px;height:97%;">    
     ...
     <!-- 明细及工具栏信息开始 -->	 
 	 <table id="account" class="easyui-datagrid" style="height:94%" data-options="toolbar:'#tb'">
    	<thead>
    		<tr>   			
    			<th data-options="field:'account_id',width:100,hidden:true">编 号</th>								
				<th data-options="field:'account_num',width:200,align:'center'">账户号</th>
				<th data-options="field:'name_cn',width:200,align:'center'">姓 名</th>
				<th data-options="field:'status',width:200,align:'center'">状 态</th>
				<th data-options="field:'regdate',width:300,align:'center'">开户时间</th>
				<th data-options="field:'regplace',width:300,align:'center'">开户银行</th>
    		</tr>
    	</thead>
     </table>	 
     <div id="tb" style="padding:5px;height:auto">
		...
		<div style="margin-bottom:5px">
			<a id="add" class="easyui-linkbutton" iconCls="icon-add"  plain="true" >开 户</a>	
			<a id="frozen" class="easyui-linkbutton" iconCls="icon-cha"  plain="true" >冻 结</a>						
			<a id="unfrozen" class="easyui-linkbutton" iconCls="icon-dui" plain="true">解 冻</a>
			<a id="cancel" class="easyui-linkbutton" iconCls="icon-print" plain="true">注 销</a>
		</div>
		<!-- 查询部分 -->
		<div>				
