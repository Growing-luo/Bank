...
<script type="text/javascript">
	...
	$(function(){
		$("#customer").datagrid({
			...
			url:"CustomerServlet?caozuo=initdatagrid"
		});
		...
		$("#add").click(function(){
			actionflag="add";
			$("#dlgcustomer").dialog("open").dialog("center").dialog("setTitle","添加客户信息");
			$("#dlgcustomer").form("clear");
		});
		...
		$("#save").click(function(){
			...
			if(actionflag=="add"){//add
				$.post("CustomerServlet?caozuo=add",
				...
				$.post("CustomerServlet?caozuo=edit&customer_id="+customer_id,
		...
		$("#edit").click(function(){
			actionflag="edit";
			var row=$("#customer").datagrid("getSelected");  //获得datagrid选中的行
			if(row){
				customer_id=row.customer_id;
				$("#dlgcustomer").dialog("open").dialog("center").dialog("setTitle","编辑客户信息");
				...
				$.messager.alert("提示","请选择要编辑的客户信息！","info");
				return;
			}
		});
		...
		$("#delete").click(function(){
			...
			$.post("CustomerServlet?caozuo=delete",
			{"customer_id":row.customer_id},
			function(data){
				var value=eval("("+data+")");
				if(value.status=="success"){
		...
		$("#query").click(function(){
			...
			$("#customer").datagrid("reload",{"condition":condition});			
		});
		...
		$("#print").click(function(){
			location.href="<%=basePath%>CustomerServlet?caozuo=print";
		});
 ...
 <body style="margin-top:10px; margin-left: 4px; margin-right:4px;height:97%;">    
     ...
     <!--  点击增加按钮弹出的对话框开始  -->
     <div id="dlgcustomer" class="easyui-dialog" style="width:360px;height:400px;padding:10px 10px" closed="true" buttons="#dlg-buttons">
	 ...
	 <div id="dlg-buttons">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton"  iconCls="icon-cancel" onclick="javascript:$('#dlgcustomer').dialog('close')" style="width:90px">关 闭</a>
	 </div>
	 <!-- 点击增加按钮弹出的对话框结束 -->
   </body>
</html>